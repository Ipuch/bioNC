...
class BiomechanicalModelJoints(GenericBiomechanicalModelJoints):
    ...
    def constraints(self, Q: NaturalCoordinates, segments: BiomechanicalModelSegments) -> np.ndarray:
        ...
        Phi_k = np.zeros(self.nb_constraints)
    ...
    def constraints_jacobian(self, Q: NaturalCoordinates, segments: BiomechanicalModelSegments) -> np.ndarray:
        ...
        K_k = np.zeros((self.nb_constraints, Q.shape[0]))
        ...
        for joint_name, joint in self.joints_with_constraints.items():
            ...
            idx_col_parent = (
                slice(12 * segments[joint.parent.name].index, 12 * (segments[joint.parent.name].index + 1))
