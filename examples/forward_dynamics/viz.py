...
class VtkFrameModel:
    ...
    def update_frame(self, Q: SegmentNaturalCoordinates):
        ...
        self.three_vectors[0].update_force(segment_jcs=[np.identity(4)], all_forces=u_vector[np.newaxis, :, np.newaxis], max_forces=[1], normalization_ratio=1)
        self.three_vectors[1].update_force(segment_jcs=[np.identity(4)], all_forces=v_vector[np.newaxis, :, np.newaxis], max_forces=[1], normalization_ratio=1)
        self.three_vectors[2].update_force(segment_jcs=[np.identity(4)], all_forces=w_vector[np.newaxis, :, np.newaxis], max_forces=[1], normalization_ratio=1)
...
class VtkGroundFrame:
    ...
    def update_frame(self):
        ...
        self.three_vectors[0].update_force(segment_jcs=[np.identity(4)], all_forces=x_vector[np.newaxis, :, np.newaxis], max_forces=[1], normalization_ratio=1)
        self.three_vectors[1].update_force(segment_jcs=[np.identity(4)], all_forces=y_vector[np.newaxis, :, np.newaxis], max_forces=[1], normalization_ratio=1)
        self.three_vectors[2].update_force(segment_jcs=[np.identity(4)], all_forces=z_vector[np.newaxis, :, np.newaxis], max_forces=[1], normalization_ratio=1)


def cheap_animation(model: BiomechanicalModel, Q: NaturalCoordinates):
    """
