...
def test_export_c3d(bionc_type):
    ...
    add_technical_markers_to_c3d(acq, model, model.Q_from_markers(markers[:, :, :]))
    add_natural_coordinate_to_c3d(acq, model, model.Q_from_markers(markers[:, :, :]))
    # delete c3d file
    os.remove(filename)
