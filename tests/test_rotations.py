...
def test_rotation_x():
    angle = np.pi / 4
    expected_matrix = np.array([[1, 0, 0], [0, np.cos(angle), -np.sin(angle)], [0, np.sin(angle), np.cos(angle)]])
    np.testing.assert_array_almost_equal(rotation_x(angle), expected_matrix)


def test_rotation_y():
    angle = np.pi / 4
    expected_matrix = np.array([[np.cos(angle), 0, np.sin(angle)], [0, 1, 0], [-np.sin(angle), 0, np.cos(angle)]])
    np.testing.assert_array_almost_equal(rotation_y(angle), expected_matrix)


def test_rotation_z():
    angle = np.pi / 4
    expected_matrix = np.array([[np.cos(angle), -np.sin(angle), 0], [np.sin(angle), np.cos(angle), 0], [0, 0, 1]])
